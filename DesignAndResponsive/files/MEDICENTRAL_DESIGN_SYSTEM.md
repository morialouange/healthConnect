...
| Marque primaire (boutons, liens, actif) | `--brand-primary` | `#2563EB` |
| Marque primaire hover | `--brand-primary-hover` | `#1D4ED8` |
| Marque primaire, fond léger (item actif sidebar) | `--brand-primary-tint` | `#EEF3FF` |
...
## 10. Corrections v2 — topbar compacte, pas de superpositions, pas de ligne décorative

Ces règles corrigent des dérives observées dans une implémentation réelle du
projet. **À appliquer immédiatement, elles remplacent toute version
antérieure du topbar :**

1. **Aucune ligne/motif décoratif sous les titres de page.** Un titre de
   page (`<h1>`) est TOUJOURS suivi directement d'un sous-texte optionnel,
   sans séparateur graphique (pas de ligne en pointillés, pas de motif
   ECG/pouls — ce motif n'existe que dans le logo, jamais ailleurs).
   Utiliser systématiquement :
   ```jsp
   <div class="page-header">
     <h1>Titre de la page</h1>
     <p>Sous-texte contextuel (optionnel)</p>
   </div>
   ```

2. **Un seul contrôle pour réduire/afficher la sidebar.** Il doit être
   unique et se trouver dans la topbar (`.sidebar-toggle-btn`, icône
   "hamburger" 32×32px). Ne jamais ajouter une seconde flèche ou un second
   bouton de repli à côté du logo dans la sidebar — c'est ce doublon qui
   crée de la confusion visuelle.

3. **Topbar compacte (56px de haut, pas 64px).** Tous les éléments de la
   topbar sont réduits par rapport à une v1 trop massive :
   - Bouton menu / icônes rondes : **32×32px** (pas 36–40px)
   - Barre de recherche : hauteur **34px**, largeur **260px**, police 13px
   - Avatar : **32×32px**, initiales en 12px
   - Rôle affiché sous le nom : 10px, MAJUSCULES, gras, couleur
     `--brand-primary` (jamais en texte gris discret — le rôle doit être
     immédiatement identifiable)
   - Un séparateur vertical fin (`border-left`) isole le bloc "nom + rôle"
     du reste des icônes.

4. **Pas de superposition d'éléments.** Cause fréquente : plusieurs
   éléments en `position:absolute`/`fixed` sans `z-index` cohérent, ou une
   topbar en `sticky` sans `isolation:isolate`. Règles obligatoires :
   - La topbar a `position:sticky; top:0; z-index:30; isolation:isolate;`
   - Aucun autre élément de la page ne doit dépasser `z-index:20`
   - Toute bordure ou ombre d'une carte doit rester **à l'intérieur** de
     son propre conteneur (`overflow` par défaut, ne jamais faire
     déborder un `box-shadow` négatif au-delà de la carte)
   - Ne jamais superposer deux `<header>`/`<nav>` sticky l'un sur l'autre

5. **Les barres de progression à 0% doivent être visuellement vides.**
   Si `.progress-bar-fill { width: 0%; }` s'applique bien en JSTL
   (`width:${valeur}%`), la barre reste vide — vérifier que le conteneur
   `.progress-bar` a un fond clair visible même quand le remplissage est
   à 0, pour ne pas donner l'impression d'un bug d'affichage.
