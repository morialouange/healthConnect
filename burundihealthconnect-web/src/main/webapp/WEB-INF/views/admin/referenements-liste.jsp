...
<c:set var="pageRoute" value="admin-referenements"/>
<c:set var="pageTitre" value="Référencements reçus"/>
<!DOCTYPE html>
        ...
        <c:if test="${not empty referenements}">
            <table class="table">
                ...
                <tr>
                    ...
                    <th><fmt:message key="table.hopital_source"/></th>
                        ...
                        <td>
                            <c:if test="${r.statut == 'EN_ATTENTE'}">
                                <form method="post" action="${pageContext.request.contextPath}/admin/referenement/accepter"
                                    ...
                                    <button type="submit" class="btn btn-small"><fmt:message key="btn.accepter"/></button>
                                </form>
                            </c:if>
                            <c:if test="${r.statut == 'ACCEPTE'}">
                                <form method="post" action="${pageContext.request.contextPath}/admin/referenement/retour"
                                      style="display:inline;">
                                    <input type="hidden" name="idRef" value="${r.idRef}">
                                    <textarea name="retour" rows="2" placeholder="<fmt:message key='placeholder.resultat.ref'/>..." required
                                              style="width:100%; margin-bottom:0.3rem;"></textarea>
                                    <button type="submit" class="btn btn-small"><fmt:message key="btn.send.retour"/></button>
                                </form>
                            </c:if>
